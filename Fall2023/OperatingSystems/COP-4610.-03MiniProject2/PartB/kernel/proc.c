...
// In proc.c, replace the scheduler function
// This function is the scheduler loop.
void scheduler(void) {
  struct proc *p;
  ...
  for (;;) {
    ...
    int highest_priority = 201; // Start with the lowest possible priority

    // Loop to find the highest priority RUNNABLE process
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    // Log the PID and priority of the process being scheduled
    cprintf("Scheduling process with PID: %d, Priority: %d\n", highest_priority_proc->pid, highest_priority_proc->priority);

    cpu->proc = highest_priority_proc;
    switchuvm(highest_priority_proc);
    highest_priority_proc->state = RUNNING;

    highest_priority_proc->ticks++;
    swtch(&cpu->scheduler, highest_priority_proc->context);
    switchkvm();

    // Log the total ticks after the process has run
    cprintf("Process with PID: %d finished running. Total Ticks: %d\n", highest_priority_proc->pid, highest_priority_proc->ticks);
