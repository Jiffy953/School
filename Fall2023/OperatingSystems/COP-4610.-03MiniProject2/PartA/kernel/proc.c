...
int sys_getpinfo(void) {
  ...
  for (int i = 0; i < NPROC; i++) {
    ...
    st->tickets[i] = ptable.proc[i].tickets;
...
found:
  ...
  p->tickets = 1; // Initialize the number of tickets
  p->ticks = 0; // Initialize the number of times scheduled
...
fork(void)
{
  ...
  *np->tf = *proc->tf;
  ...
  // Child inherits the number of tickets from the parent
  np->tickets = proc->tickets;
  // Initialize the ticks for the child process
  np->ticks = 0;
...
// Simple LCG function for random numbers
unsigned int rand(void) {
  randstate = randstate * 1664525 + 1013904223;
  return (unsigned int)(randstate >> 16); // Returning the upper 16 bits
}
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // If there are no tickets, no process is RUNNABLE, so continue
    if(totalTickets == 0)
      continue;

    // Generate a winning lottery ticket
    lotteryWinner = rand() % totalTickets;

    // Loop over process table looking for the process with the winning ticket
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;

      // Subtract the process's tickets from the lottery winner until
      // the winner is found
      if((lotteryWinner -= p->tickets) < 0){
        // This process is the winner; increment its ticks and run it.
        p->ticks++; // Increment the ticks counter for the winning process
        proc = p;
        switchuvm(p);
        p->state = RUNNING;
        ...
        break; // Break after running the chosen process
      }
    }
    release(&ptable.lock);
  }
